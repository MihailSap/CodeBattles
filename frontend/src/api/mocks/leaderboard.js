...
const getMemberships = (id) => {
  ...
  const projectIds = [
    (id % MOCK_LEADERBOARD_PROJECTS.length) + 1,
    ((id + 3) % MOCK_LEADERBOARD_PROJECTS.length) + 1
  ];
...
const createStats = (id, index, periodMultiplier) => {
  ...
  return {
    ...
    fixedCommentsPercent: Math.max(0, Math.min(100, Math.round((isCurrentMockUser ? 18 : 98 - index * 0.38) * periodMultiplier + (1 - periodMultiplier) * 12))),
    ...
    reviewDepthPercent: Math.max(0, Math.min(100, Math.round((isCurrentMockUser ? 22 : 99 - index * 0.33) * periodMultiplier + (1 - periodMultiplier) * 15))),
