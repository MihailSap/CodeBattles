...
- Загрузка решений вручную, отдельными файлами, ZIP-архивом или через public GitHub pull request из единой формы.
- Рабочее пространство решения с деревом файлов и просмотром кода, синхронизированным с темой сайта.
...
- Профиль пользователя, настройки уведомлений, стек навыков и привязка/отвязка GitHub-аккаунта.
...
Frontend переведен на `.ts/.tsx` и проверяется через strict TypeScript: `tsconfig.app.json` содержит `strict: true`, без `noCheck`, без `allowJs` и без отключения `noImplicitAny`/`strictNullChecks`. Для локальной проверки используйте `npm run typecheck`, `npm run lint`, `npm run build` и `npm run format:check` из каталога `frontend/`.
