...
Формы frontend-приложения валидируются через React Hook Form и Zod. Доменные схемы лежат в `entities/*/lib/validation.js`, схемы фичевых форм — в `features/*/model`.

Качество frontend-кода поддерживается через ESLint Flat Config и Prettier. Конфигурации находятся в `frontend/eslint.config.js`, `frontend/.prettierrc` и `frontend/.prettierignore`.
